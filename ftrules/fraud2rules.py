...
class FraudToRules(BaseEstimator):
    ...
    Attributes
    ----------
    rules_ : list of tuples of the form (rule, precision).
        ...
        precisions.
        All rules are saved, even the non-selected one. The selected
        rules (the ones used in the ``predict`` method) can be obtained
        through rules_[:n_estimators].
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        for estimator, samples, features in zip(self.estimators_,
            ...
            if sum(mask) == 0:
                ...
                mask = samples
            rules_from_tree = self._tree_to_rules(estimator,
                                                  self.feature_names_)
    ...
    def decision_function(self, X):
        ...
        # "bigger is better" convention (here less abnormal)
        scores = -scores
        return scores
    ...
    def _eval_rule_perf(self, rule, X, y):
        ...
        if len(detected_index) <= 1:
            ...
            return y[list(X.query(rule).index)].mean()
