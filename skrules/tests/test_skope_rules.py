...
def test_skope_rules():
    ...
    grid = ParameterGrid([{
        ...
        "deduplication_criterion": ['f1', 'mcc'],
        "myfunc": [None],
        "n_estimators": [1],
        "max_samples": [0.5, 4],
        ...
        "n_jobs": [-1, 2]},
        # grid with myfunc parameter
        {
        "feature_names": [None, ['a', 'b']],
        "filtering_criteria": [{'precision': 0., 'recall': 0., 'myfunc': 0}],
        "deduplication_criterion": ['f1', 'mcc', 'myfunc'],
        "myfunc": [lambda tn, fp, fn, tp: (tn+tp-fp+fn)],
        "n_estimators": [1],
        "max_samples": [0.5, 4],
        "max_samples_features": [0.5, 2],
        "bootstrap": [True, False],
...
def test_skope_rules_error():
    ...
    with assert_raises(ValueError):
        SkopeRules(deduplication_criterion='foobar').fit(X, y)
    # check myfunc errors
    with assert_raises(TypeError):
